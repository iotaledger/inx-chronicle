...
mod test_rand {
    ...
    async fn test_ledger_updates() {
        ...
        let unspent_outputs = std::iter::repeat_with(|| Output::rand(&protocol_params))
            .take(100)
            .map(|mut output| {
                if let Output::Basic(o) = &mut output {
                    o.address_unlock_condition.address = address;
        ...
        collection.merge_into_ledger_updates(1.into()).await.unwrap();
        collection.merge_into_ledger_updates(2.into()).await.unwrap();

        let outputs_map = spent_outputs
            .iter()
            .filter(|o| o.spent_metadata.spent.milestone_index == 2)
            .map(|o| (&o.output, true))

...
//     }): Json<CreateProofResponse>,
// ) -> ApiResult<ValidateProofResponse> { // Extract block, milestone, and audit path. let block =
//   iota_sdk::types::block::Block::try_from_dto(block) .map_err(|_|
//   RequestError::PoI(poi::RequestError::MalformedJsonBlock))?; let block_id = block.id().into(); let milestone =
//   iota_sdk::types::block::payload::milestone::MilestonePayload::try_from_dto(milestone) .map_err(|_|
//   RequestError::PoI(poi::RequestError::MalformedJsonMilestone))?; let milestone_index = milestone.essence().index();
//   let proof = MerkleAuditPath::try_from(merkle_path) .map_err(|_|
//   RequestError::PoI(poi::RequestError::MalformedJsonAuditPath))?;
...
//             valid: proof.contains_block_id(&block_id) && *proof.hash() ==
...
//         return Err(RequestError::PoI(poi::RequestError::BlockNotConfirmed(block_id.to_hex())).into());
//     } else if block_metadata.inclusion_state != LedgerInclusionState::Included {
...
//   iota_sdk::types::block::payload::milestone::MilestonePayload::try_from_dto(milestone) .map_err(|_|
//   RequestError::PoI(poi::RequestError::MalformedJsonMilestone))?; let milestone_index = milestone.essence().index();
//   let audit_path = MerkleAuditPath::try_from(audit_path) .map_err(|_|
//   RequestError::PoI(poi::RequestError::MalformedJsonAuditPath))?;
...
// ) -> Result<Vec<String>, CorruptStateError> { key_ranges.sort(); let mut public_keys =
//   HashSet::with_capacity(key_ranges.len()); for key_range in key_ranges.iter() { match (key_range.start,
//   key_range.end) { (start, _) if start > index => break, (start, end) if index <= end || start == end => { let
//   public_key_raw = prefix_hex::decode::<Vec<_>>(&key_range.public_key) .map_err(|_|
//   CorruptStateError::PoI(poi::CorruptStateError::DecodePublicKey))?; let public_key_hex =
//   hex::encode(public_key_raw); public_keys.insert(public_key_hex); } (_, _) => continue, } }

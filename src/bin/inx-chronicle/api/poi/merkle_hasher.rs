...
impl MerkleHasher {
    ...
    pub(crate) fn leaf<D: Default + Digest>(value: &impl AsRef<[u8]>) -> Output<D> {
    ...
    pub(crate) fn node<D: Default + Digest>(left: Output<D>, right: Output<D>) -> Output<D> {
        let mut hasher = D::default();
    ...
    pub fn digest<D: Default + Digest>(value: &[impl AsRef<[u8]>]) -> Output<D> {
        match value {
            ...
            _ => {
                let (left, right) = value.split_at(largest_power_of_two(value.len()));
                Self::node::<D>(Self::digest::<D>(left), Self::digest::<D>(right))
            }
        }
    }
}

...
use super::{
    ...
    responses::{Record, TransactionResponse, Transfer, *},
};
use crate::{
    api::{
        error::APIError,
...
async fn messages_query(
    ...
    let MessagesQuery { index, included: _ } = &query;
...
async fn outputs_query(
    ...
    let OutputsQuery { address, included } = &query;
    let start_milestone = start_milestone(&database, start_timestamp).await?;
    let end_milestone = end_milestone(&database, end_timestamp).await?;
    ...
    Ok(TransactionResponse {
        ...
        outputs: match transaction.payload() {
            Some(Payload::Transaction(t)) => match t.essence() {
                Essence::Regular(e) => e.outputs().iter().map(|o| serde_json::to_value(o).unwrap()).collect(),
            },
            _ => unreachable!(),
        },
        inputs: match transaction.payload() {
            Some(Payload::Transaction(t)) => match t.essence() {
                Essence::Regular(e) => e.inputs().iter().map(|o| serde_json::to_value(o).unwrap()).collect(),
            },
            _ => unreachable!(),
        },
    })

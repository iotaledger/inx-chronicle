...
use crate::{
    db::{
        mongodb::collections::{
            BlockCollection, ConfigurationUpdateCollection, OutputCollection, ProtocolUpdateCollection,
        },
        MongoDb,
    },
...
//     ) -> Result<BoxStream<Result<BlockData, Self::Error>>, Self::Error> { Ok(Box::pin(
//       self.collection::<BlockCollection>() .get_referenced_blocks_in_white_flag_order_stream(index) .await?
//       .map_err(|e| e.into()) .map_ok(|(block_id, block, raw, metadata)| BlockData { block_id, block, raw, metadata,
//       }), ))
//     }

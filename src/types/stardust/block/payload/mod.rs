...
mod rand {
    ...
    impl Payload {
        ...
        pub fn rand_spending_transaction(ctx: &bee_block_stardust::protocol::ProtocolParameters) -> (Self, Input) {
            let (payload, input) = TransactionPayload::rand_spending(ctx);
            (Self::Transaction(Box::new(payload)), input)
        }
